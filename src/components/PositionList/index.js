...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name taalswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name taalswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name taalswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  taalswap_RETURN: 'taalswap_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.taalswap_RETURN) {
          return p0?.taalswap?.return > p1?.taalswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.taalswap_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.taalswap_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.taalswap_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
